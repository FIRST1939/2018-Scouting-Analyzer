...
def TeamStats(TeamDf):
    ...
    MatchCount = pd.pivot_table(TeamDf, values = ['totalmatches', 'endClimbedType'], index = 'team', aggfunc = np.count_nonzero)
...
def PickList(TeamDf, PivotDf, lastMatch):
    ...
    earlytelepivot = pd.pivot_table(earlyDf, values = ['avgtelecubes'], index = 'team', aggfunc = np.average)
    latetelepivot = pd.pivot_table(lateDf, values = ['avgtelecubes'], index = 'team', aggfunc = np.average)
    ...
    deltaDf['change'] = deltaDf['avgtelecubes_late'] - deltaDf['avgtelecubes_early']
